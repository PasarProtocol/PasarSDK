...
export class AssistService {
    ...
    public async getAllListedItems(collectionAddr = '', pageNum = 1, pageSize = 10): Promise<NftListInfo> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getDetailedCollectibles?collectionType=${collectionAddr}&tokenType=&status=All&itemType=All&adult=false&minPrice=&maxPrice=&order=0&marketPlace=0&keyword=&pageNum=${pageNum}&pageSize=${pageSize}`).then (async response => {
    ...
    public async getCollectionInfo(collectionAddr:string, chainType: ChainType): Promise<CollectionInfo> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getCollection/${collectionAddr}?marketPlace=${getChainTypeNumber(chainType)}`).then (async response => {
     ...
     */
     public async getItemByTokenId(tokenId:string, baseToken:string): Promise<NftItem> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getCollectibleByTokenId/${tokenId}/${baseToken}`).then(async response => {
            let data = await response.json();
            if (data['code'] != 200) {
    ...
    public async getOwnedCollections(walletAddr: string): Promise<CollectionInfo[]> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getCollectionByOwner/${walletAddr}?marketPlace=1`).then(async response => {
            let data = await response.json();
            if (data['code'] != 200) {
     ...
     */
     public async getListedItems(walletAddr: string): Promise<NftItem[]> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getListedCollectiblesByAddress/${walletAddr}?orderType=0`).then(async response => {
            let data = await response.json();
            if (data['code'] != 200) {
     ...
     */
     public async getOwnedItems(walletAddr: string): Promise<NftItem[]> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getOwnCollectiblesByAddress/${walletAddr}?orderType=0`).then(async response => {
            let data = await response.json();
            if (data['code'] != 200) {
                throw new Error("Call API to fetch bidding NFT failed");
            }
     ...
     */
     public async getCreatedItems(walletAddr: string): Promise<NftItem[]> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getCreatedCollectiblesByAddress/${walletAddr}?orderType=0`).then(async response => {
            let data = await response.json();
            if (data['code'] != 200) {
                throw new Error("Call API to fetch bidding NFT failed");
            }
     ...
     */
     public async getBiddingItems(walletAddr: string): Promise<NftItem[]> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getBidCollectiblesByAddress/${walletAddr}?orderType=0`).then(async response => {
            let data = await response.json();
            if (data['code'] != 200) {
                throw new Error("Call API to fetch bidding NFT failed");
            }
     ...
     */
     public async getSoldItems(walletAddr: string): Promise<NftItem[]> {
        return await fetch(`${this.baseUrl}/api/v2/sticker/getSoldCollectiblesByAddress/${walletAddr}?orderType=0`).then(async response => {
            let data = await response.json();
            if (data['code'] != 200) {
